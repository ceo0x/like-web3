...
class LikeWeb3 extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.web3 = new Web3(LikeWeb3.anyProvider(opts.provider, opts.options));
  ...
  */
  ...
  estimateGasPrice() {
    return this.estimateGas({ gasLimit: -1 }).then(
      (estimated) => estimated.gasPrice
    );
  }

  // const gasLimit = await web3.estimateGasLimit({ from, to, value, data })
  estimateGasLimit({ from, to, value, data }) {
    return this.estimateGas({ gasPrice: -1, from, to, value, data }).then(
      (estimated) => estimated.gasLimit
    );
  }
  ...
  async estimateGas({
    ...
    if (gasPrice && gasPriceWei)
      throw new Error("should only use one, either gasPrice or gasPriceWei");
    if (gasLimit && gasLimitWei)
      throw new Error("should only use one, either gasLimit or gasLimitWei");

    // wei to normal
    gasPrice = gasPriceWei
      ? this.web3.utils.fromWei(gasPriceWei, "gwei")
      : gasPrice;
    gasLimit = gasLimitWei
      ? this.web3.utils.fromWei(gasLimitWei, "gwei")
      : gasLimit;
    ...
    promises.push(
      gasLimit ? null : this.web3.eth.estimateGas({ from, to, value, data })
    );
  ...
  trunc(amount, decimals) {
    ...
    const substrLength =
      decimalPos === -1 ? value.length : decimalPos + decimals;
  ...
  */
  subscribe(type, opts = {}) {
    if (type === "pendings") {
      ...
      this.subscriptions.pendings = this.web3.eth.subscribe(
        "pendingTransactions",
        (error, transactionHash) => {
          ...
          batch.add(
            this.web3.eth.getTransaction.request(
              transactionHash,
              (error, tx) => {
                if (error) {
          ...
          if (
            batch.requests.length >= batchCount ||
            Date.now() - batch.$started >= batchDelay
          ) {
            batch.execute();
            this.batches.pendings = null;
          } else {
  ...
  unsubscribe(type) {
    return new Promise((resolve, reject) => {
      if (!type) {
        Promise.all([this.unsubscribe("pendings"), this.unsubscribe("blocks")])
          .then(resolve)
          .catch(reject);
        return;
      }
  ...
  async getPair(factory, pair) {
    ...
    const factoryContract = new ContractETH(
  ...
  async approve(
    contract,
    { spender, amount, from, nonce, gasPrice, gasLimit, privateKey }
  ) {
    const CONTRACT = Contracts.parse(contract);
  ...
  async getReserves(factory, pair) {
    ...
    const pairContract = new ContractETH(
      Contracts.PANCAKESWAP_PAIR.abi,
      pairAddress
    );
    ...
    const orderedPair = new Decimal(pair[0]).lt(pair[1])
      ? [pair[0], pair[1]]
      : [pair[1], pair[0]];
    ...
    reserves = {
      ...
      changed: "",
      [pair[0]]: this.fromWei(
        pair[0] === orderedPair[0] ? reserves._reserve0 : reserves._reserve1,
        decimals0
      ),
      [pair[1]]: this.fromWei(
        pair[0] === orderedPair[0] ? reserves._reserve1 : reserves._reserve0,
        decimals1
      ),
  ...
  sync(swap, reserve) {
    ...
    if (
      swap.amountOutMin &&
      new Decimal(swap.amountOut).lt(swap.amountOutMin)
    ) {
      throw new Error(
        "sync: amountOut (" +
          swap.amountOut +
          ...
          swap.amountOutMin +
          ") by " +
          new Decimal(swap.amountOutMin).minus(swap.amountOut).toFixed()
      );
    }

    // update liquidity
    synced[swap.path[0]] = new Decimal(synced[swap.path[0]])
      .add(amountIn)
      .toFixed();
    ...
    const isFirstWBNB =
      swap.path[0] === "0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c";
    synced.price = this.fromWei(
      this.quote(
    ...
    synced.changed = new Decimal(synced.price)
      .div(reserve.price)
      .minus(1.0)
      .toFixed();

    // update liquidity
    synced[swap.path[1]] = new Decimal(synced[swap.path[1]])
      .minus(amountOut)
      .toFixed();

    // from wei
    synced[swap.path[0]] = this.fromWei(
      synced[swap.path[0]],
      synced.decimals[0]
    );
    synced[swap.path[1]] = this.fromWei(
      synced[swap.path[1]],
      synced.decimals[1]
    );
    return synced;
  ...
  quote(amountA, reserveA, reserveB) {
    ...
    if (!(reserveA > 0 && reserveB > 0))
      throw new Error("PancakeLibrary: INSUFFICIENT_LIQUIDITY");
    const amountB = new Decimal(new Decimal(amountA).mul(reserveB).toFixed())
      .div(reserveA)
      .toFixed();
  ...
  getAmountIn(factory, amountOut, reserveIn, reserveOut) {
    if (!(amountOut > 0))
      throw new Error("PancakeLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    const denominator = new Decimal(
      new Decimal(reserveOut).sub(new Decimal(amountOut))
    ).mul(new Decimal(1000).minus(FEE).toFixed()); // 1000 => 997.5
    const amountIn = new Decimal(numerator)
      .div(new Decimal(denominator))
      .add(1)
      .toFixed();
  ...
  getAmountOut(factory, amountIn, reserveIn, reserveOut) {
    if (!(amountIn > 0))
      throw new Error("PancakeLibrary: INSUFFICIENT_INPUT_AMOUNT");
    if (!(reserveIn > 0 && reserveOut > 0))
      throw new Error("PancakeLibrary: INSUFFICIENT_LIQUIDITY");
    const FACTORY = Contracts.parse(factory);
    const FEE = new Decimal(FACTORY.fee).mul(1000).toFixed(); // 0.0025 => 2.5
    const amountInWithFee = new Decimal(amountIn).mul(
      new Decimal(1000).minus(FEE).toFixed()
    ); // 1000 => 997.5
    const numerator = new Decimal(amountInWithFee).mul(reserveOut);
    const denominator = new Decimal(reserveIn)
      .mul(1000)
      .add(new Decimal(amountInWithFee));
    const amountOut = new Decimal(new Decimal(numerator))
      .div(new Decimal(denominator))
      .toFixed();
    return this.trunc(amountOut);
  }

  static anyProvider(url, options = {}) {
    // support for websocket
    if (url.startsWith("wss://") || url.startsWith("ws://")) {
      return new Web3.providers.WebsocketProvider(url, options);
    }
  ...
  parsePrivateKey(privateKey) {
    const has =
      privateKey.indexOf("0x") === 0 || privateKey.indexOf("0X") === 0;
